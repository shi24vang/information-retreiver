...
void QNA_tool::query(string question, string filename) {
    ...
    if (!analysis.first || analysis.second == 0) {
        std::cerr << "No paragraphs available for summarization.\n";
        return;
    }
    const char* api_key = std::getenv("OPENAI_API_KEY");
    if (!api_key) {
        std::cerr << "OPENAI_API_KEY not set; skipping external summary step.\n";
        return;
    }
...
void QNA_tool::query_llm(string filename, Node* root, int k, string API_KEY, string question) {
    ...
    query_file << "Excerpts from the corpus are saved in paragraph_<i>.txt files.\n";
    query_file << "Using those paragraphs, answer: " << question;
    query_file.close();
    std::ostringstream cmd;
    cmd << "python3 " << filename << " " << API_KEY << " " << count << " query.txt";
